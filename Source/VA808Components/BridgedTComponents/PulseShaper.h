/*
  ==============================================================================
    ...
    Class that represents the pulse shaper input of the bridged-T network.
    Contains two classes for managing transfer functions:
        1) between the pulse shaper input and network output
                        V_plus ---> V_bt1
        2) between the pulse shaper input and the intermediate node
                        V_plus ---> V_c1
  ==============================================================================
*/
...
public:
    void calculateCoefficients(float K, float r_prl1, float r161, float r167, float r170, float c41, float c42)
    {
        ...
        B1 = (2.0f * beta0 - 2 * K * K * beta2) / A0;
        B2 = (K * K * beta2 - K * beta1 + beta0) / A0;
        A1 = (2.0f * alpha0 - 2 * K * K * alpha2) / A0;
...
private:
    /// coefficients of the discretized transfer functions for bridged-T network, for V_plus ---> V_bt1
    float A0 = 0.0f;
    float B0 = 0.0f;
    float B1 = 0.0f;
    float B2 = 0.0f;
...
private:
    /// coefficients of the discretized transfer functions for bridged-T network, for V_plus ---> V_c1
    float A0 = 0.0f;
    float B0 = 0.0f;
    float B1 = 0.0f;
    float B2 = 0.0f;
